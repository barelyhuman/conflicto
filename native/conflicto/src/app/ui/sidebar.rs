...
use crate::ui_kit::{Child, ControlSize, HStack, Insets, Space};

pub fn show(ui: &mut Ui, app: &mut ConflictoApp, u: &UiVars) {
    ui.set_min_size(ui.available_size());

    let control_h = ControlSize::TOOLBAR_INNER;
    HStack::new()
        .gap(Space::Sm)
        .padding(Insets::from_space(Space::Md, Space::Sm))
        .frame_height(control_h)
        .background(rgb(u.bg_surface))
        .show(
            ui,
            &[Child::flex(1.0), Child::fixed(control_h)],
            |ui, i| match i {
                0 => {
                    ...
                    egui::ComboBox::from_id_salt("repo_switcher")
                        ...
                        .show_ui(ui, |ui| {
                            ...
                            for r in recent {
                                ui.horizontal(|ui| {
                                    let selected = current_root
                                        .as_ref()
                                        .is_some_and(|root| root == &r.root);
